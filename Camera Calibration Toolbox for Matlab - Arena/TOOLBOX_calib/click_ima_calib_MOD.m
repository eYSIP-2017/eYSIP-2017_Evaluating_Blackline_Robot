...
KHpts = [0;0;1];
for i=2:(n_sq_x+1)*(n_sq_y+1)
    [KHi,KHj] = ind2sub(KHsize,i);
%     if (KHi==1)||(KHj==1)||(KHi==n_sq_x+1)||(KHj==n_sq_y+1)||(KHj==2)||(KHj==n_sq_y) ||(KHi==n_sq_x) ||(KHj==n_sq_y-1)
    if (KHi==1)||(KHi==n_sq_x)||(KHi==n_sq_x+1)||(KHj==1)||(KHj==2)||(KHj==3) ||(KHj==n_sq_y) ||(KHj==n_sq_y+1)
%     if (KHi==n_sq_x)||(KHj==n_sq_y-1)
        KHpts = [KHpts pts(:,i)];
    end
end
pts = KHpts;
...
Xi = reshape(([0:n_sq_x]*dX)'*ones(1,n_sq_y+1),(n_sq_x+1)*(n_sq_y+1),1)';
Yi = reshape(ones(n_sq_x+1,1)*[n_sq_y:-1:0]*dY,(n_sq_x+1)*(n_sq_y+1),1)';
Zi = zeros(1,(n_sq_x+1)*(n_sq_y+1));
