function [x,X,n_sq_x,n_sq_y,ind_orig,ind_x,ind_y,KW_ExCompOrigin] = extract_grid(I,wintx,winty,fc,cc,kc,dX,dY,xr,yr,click_mode);
...
if nargin < 2,
    ...
    wintx = input('wintx ([] = 25) = ');
    if isempty(wintx), wintx = 25; end;
    wintx = round(wintx);
    winty = input('winty ([] = 25) = ');
    if isempty(winty), winty = 25; end;
    winty = round(winty);
...
while need_to_click,
    ...
    if (n_sq_x1~=n_sq_x2)|(n_sq_y1~=n_sq_y2),
        ...
        if ~click_mode,
            ...
            n_sq_x = input('Number of squares along the X direction ([]=15) = '); %6
            if isempty(n_sq_x), n_sq_x = 15; end;
            n_sq_y = input('Number of squares along the Y direction ([]=3) = '); %6
            if isempty(n_sq_y), n_sq_y = 3; end; 
            need_to_click = 0;
...
if ~exist('dX')|~exist('dY'),
    ...
    dX = input(['Size dX of each square along the X direction ([]=100mm) = ']);
    dY = input(['Size dY of each square along the Y direction ([]=100mm) = ']);
    if isempty(dX), dX = 100; end;
    if isempty(dY), dY = 100; end;
...
% %% START Code by khalid
% KHsize = [(n_sq_x+1) (n_sq_y+1)];
% KHx = [];
% KHX = [];
% for i=1:size(x,2)
%     [KHi,KHj] = ind2sub(KHsize,i);
% %     if (KHi==1)||(KHi==2)||(KHi==n_sq_x-1)||(KHi==n_sq_x)||(KHi==n_sq_x+1)||(KHj==1)||(KHj==n_sq_y)||(KHj==n_sq_y+1)
%     if (KHi==1)||(KHi==2)||(KHi==n_sq_x-1)||(KHi==n_sq_x)||(KHi==n_sq_x+1)||(KHj==1)||(KHj==n_sq_y+1)
%         KHx = [KHx x(:,i)];
%         KHX = [KHX X(:,i)];
%     end
% end
